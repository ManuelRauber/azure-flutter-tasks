...
* Select the `target` platform. Options are: `apk` (default), `aab`, `ios`, `ipa`, `web`, `mobile` (all mobile platforms only), `windows`, `macos`, `linux`, `desktop` (all desktop platforms only) , `all` (all platforms).
